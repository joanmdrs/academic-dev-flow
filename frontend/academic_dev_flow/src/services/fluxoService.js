...
export const criarFluxo = async (dados) => {
    try {
        const response = await api.post("/fluxo/", dados)
...
export const buscarFluxoPeloNome = async (parametro) => {
    try {
        const response = await api.get(`/fluxo/?search=${encodeURIComponent(parametro)}`)
        return response
    } catch (error) {
...
export const buscarFluxoPeloId =  async (fluxo_id) => {
    try {
        const response = await api.get(`fluxo/${encodeURIComponent(fluxo_id)}/`) 
        return response
    } catch (error) {
...
export const atualizarFluxo = async (dadosFluxo, idFluxo) => {
    try {
        const response = await api.patch(`fluxo/${encodeURIComponent(idFluxo)}/`, dadosFluxo)
...
export const excluirFluxo = async (idFluxo) => {
    try {
        const response = await api.delete(`fluxo/${encodeURIComponent(idFluxo)}/`)
...
export const listarFluxos = async () => {
    try {
        const response = await api.get('fluxo/')
        return response
    } catch (error) {
        return handleError(error, ERROR_MESSAGE_ON_SEARCHING)
    }
