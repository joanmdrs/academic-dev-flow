...
class FiltrarArtefatosPorMembroEPorProjetoView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if not id_membro and not id_projeto:
                return self._response_error('O ID do membro e o ID do projeto não foram fornecidos!', 'INVALID_INPUT')

            # Lógica principal de busca de artefatos
            artefatos, error = self._buscar_artefatos(id_membro, id_projeto)
            if error:
                return self._response_error(error['message'], error['code'])

            # Retorna a lista de artefatos serializada
            return self._response_success(artefatos)

        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    def _buscar_artefatos(self, id_membro, id_projeto):
        """
        Busca os artefatos com base nos IDs fornecidos.
        Retorna (artefatos, None) em caso de sucesso ou (None, error) em caso de erro.
        """
        if id_membro and not id_projeto:
            return self._artefatos_por_membro(id_membro)

        if id_projeto and not id_membro:
            return self._artefatos_por_projeto(id_projeto)

        if id_membro and id_projeto:
            return self._artefatos_por_membro_e_projeto(id_membro, id_projeto)

        return None, {'message': 'Nenhum critério de filtro válido foi fornecido.', 'code': 'INVALID_FILTER'}

    def _artefatos_por_membro(self, id_membro):
        membros_projeto = MembroProjeto.objects.filter(membro=id_membro)
        if not membros_projeto.exists():
            return None, {
                'message': 'O membro não está vinculado a nenhum projeto',
        ...
        return artefatos, None

    def _artefatos_por_projeto(self, id_projeto):
        artefatos = Artefato.objects.filter(projeto=id_projeto).distinct().order_by('id')
        return artefatos, None

    def _artefatos_por_membro_e_projeto(self, id_membro, id_projeto):
        ...
        artefatos = Artefato.objects.filter(
            projeto=id_projeto, membros__in=membros_projeto
        ).distinct().order_by('id')
        return artefatos, None

    def _response_success(self, artefatos):
        if not artefatos.exists():
            return Response([], status=status.HTTP_200_OK)
    ...
    def _response_error(self, message, code):
