...
class CadastrarMembroView(APIView):
    ...
    def post(self, request):
        user_created = None  # Inicializa a variável para evitar o erro
        try:
            user_data = request.data.get('usuario', {}) 
            member_data = request.data.get('membro', {})

            if not user_data.get('username') or not user_data.get('password'):
                return Response({'error': 'Campo usuário/senha inválidos!'}, status=status.HTTP_400_BAD_REQUEST)

            if not member_data.get('grupo'):
                return Response({'error': 'O grupo de usuário não foi fornecido!'}, status=status.HTTP_400_BAD_REQUEST)

            if not member_data.get('nome'):
                return Response({'error': 'O campo nome não pode ser vazio!'}, status=status.HTTP_400_BAD_REQUEST)

            if not member_data.get('email'):
                return Response({'error': 'O campo e-mail não pode ser vazio!'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not group:
                return Response({'error': 'O grupo de usuário não foi localizado!'}, status=status.HTTP_404_NOT_FOUND)
            ...
            if user_created:  # Verifica se o usuário foi criado antes de tentar deletá-lo
                user_created.delete()
...
class BuscarMembroPorIdUsuarioView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({'error': 'Membro não encorado'}, status=status.HTTP_404_NOT_FOUND)
...
class AtualizarMembroView(APIView):
    ...
    def patch(self, request):
        try:
            ### Obtendo os dados e verificando a existência
            id_membro = request.GET.get('id_membro', None)
            ...
            ### Passando as novas informações do membro
